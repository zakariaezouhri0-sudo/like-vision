            ...
            <Card className="rounded-[32px] bg-white border-none shadow-sm overflow-hidden">
              ...
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                  ...
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase ml-1">Nom Complet</Label><div className="relative"><User className="absolute left-4 top-3.5 h-4 w-4 text-primary/30" /><Input className={cn("h-12 pl-11 rounded-xl bg-slate-50 border-none shadow-inner font-bold", isSessionClosed && !canBypassLock && "opacity-50")} placeholder="M. Mohamed Alami..." value={clientName} onChange={e => handleNameChange(e.target.value)} readOnly={isSessionClosed && !canBypassLock} /></div></div>
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase ml-1">N° BON</Label><div className="relative"><ClipboardList className="absolute left-4 top-3.5 h-4 w-4 text-primary/30" /><Input className={cn("h-12 pl-11 rounded-xl bg-slate-50 border-none shadow-inner font-black text-primary", isSessionClosed && !canBypassLock && "opacity-50")} placeholder="Ex: 2472" value={bonNumber} onChange={e => setBonNumber(e.target.value)} readOnly={isSessionClosed && !canBypassLock} /></div><div className="flex items-center space-x-2 mt-2 px-1"><Checkbox id="fromDoctor" checked={fromDoctor} onCheckedChange={(v) => setFromDoctor(!!v)} disabled={isSessionClosed && !canBypassLock} /><label htmlFor="fromDoctor" className="text-[10px] font-black uppercase cursor-pointer flex items-center gap-1.5 text-slate-500"><Stethoscope className="h-3 w-3" /> MÉDECIN</label></div></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start"><div className="space-y-2"><Label className="text-[10px] font-black uppercase ml-1">Mutuelle</Label><Select value={mutuelle} onValueChange={setMutuelle} disabled={isSessionClosed && !canBypassLock}><SelectTrigger className={cn("h-12 rounded-xl bg-slate-50 border-none shadow-inner font-bold", isSessionClosed && !canBypassLock && "opacity-50")}><SelectValue /></SelectTrigger><SelectContent className="rounded-xl">{MUTUELLES.map(m => <SelectItem key={m} value={m} className="font-bold">{m}</SelectItem>)}</SelectContent></Select></div>
                  ...
                  {isAdminOrPrepa && activeEditId && (<div className="space-y-2 animate-in fade-in slide-in-from-top-1"><Label className="text-[10px] font-black uppercase ml-1 text-destructive">Correction N° Document (ADMIN)</Label><div className="relative"><FileText className="absolute left-4 top-3.5 h-4 w-4 text-destructive/30" /><Input className="h-12 pl-11 rounded-xl bg-red-50 border-2 border-red-100 font-black text-destructive" value={editableInvoiceId} onChange={e => setEditableInvoiceId(e.target.value.toUpperCase())} /></div></div>)}
                </div>
              </CardContent>
            </Card>
