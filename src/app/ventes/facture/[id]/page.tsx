    ...
    <div className="pdf-a5-portrait bg-white flex flex-col p-[10mm] relative">
      ...
      <div className="flex justify-between items-start mb-10 pb-6 border-b-2 border-primary/20">
        ...
        <div className="text-right">
          <div className="bg-primary text-white px-4 py-1.5 rounded-sm inline-block mb-3">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10 bg-slate-50 p-5 rounded-2xl border border-slate-100">
        <div className="space-y-1.5 flex flex-col items-center text-center">
          ...
          <p className="text-[12px] font-black text-slate-900 leading-none uppercase truncate w-full px-2">{clientName}</p>
        </div>
        <div className="space-y-1.5 border-x border-slate-200 px-4 flex flex-col items-center text-center">
          <div className="flex items-center justify-center gap-1 text-[9px] font-black text-primary uppercase opacity-60 tracking-widest">
            <Phone className="h-3 w-3" /> Téléphone
          </div>
          <p className="text-[12px] font-black text-slate-900 leading-none">{formatPhoneNumber(clientPhone)}</p>
        </div>
        <div className="space-y-1.5 flex flex-col items-center text-center">
          <div className="flex items-center justify-center gap-1 text-[9px] font-black text-primary uppercase opacity-60 tracking-widest">
            <ShieldCheck className="h-3 w-3" /> Mutuelle
          </div>
          <p className="text-[12px] font-black text-primary leading-none uppercase">{mutuelle}</p>
        </div>
      </div>

      {/* Prescription */}
      <div className="mb-12">
        <h3 className="text-[10px] font-black uppercase mb-3 text-primary tracking-[0.2em] flex items-center gap-3">
        ...
        <table className="w-full text-[12px] border-collapse">
          <thead>
            <tr className="bg-primary text-white">
              <th className="border border-primary p-2.5 text-left text-[9px] uppercase font-black tracking-widest">Oeil</th>
              <th className="border border-primary p-2.5 text-center text-[9px] uppercase font-black tracking-widest">Sphère</th>
              <th className="border border-primary p-2.5 text-center text-[9px] uppercase font-black tracking-widest">Cylindre</th>
              <th className="border border-primary p-2.5 text-center text-[9px] uppercase font-black tracking-widest">Axe</th>
            </tr>
          </thead>
          <tbody>
            <tr className="bg-white">
              <td className="border border-slate-200 p-2.5 font-black text-slate-700 bg-slate-50/50">Oeil Droit (OD)</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{od.sph}</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{od.cyl}</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{od.axe}</td>
            </tr>
            <tr className="bg-white">
              <td className="border border-slate-200 p-2.5 font-black text-slate-700 bg-slate-50/50">Oeil Gauche (OG)</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{og.sph}</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{og.cyl}</td>
              <td className="border border-slate-200 p-2.5 text-center font-bold">{og.axe}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="mb-12 grid grid-cols-2 gap-6">
        {monture && (
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col gap-1">
            ...
            <p className="text-[11px] font-bold text-slate-800 uppercase">{monture}</p>
          </div>
        )}
        {verres && (
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col gap-1">
            <p className="text-[9px] font-black uppercase text-primary/60 tracking-widest">Verres</p>
            <p className="text-[11px] font-bold text-slate-800 uppercase">{verres}</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-auto">
        <div className="flex justify-between items-end gap-10 border-t-2 border-slate-100 pt-10">
          <div className="flex-1 space-y-3 bg-slate-50 p-5 rounded-[24px]">
            ...
            <div className="flex justify-between text-[12px] text-slate-900 font-black border-t border-slate-200 pt-2.5">
              <span>Total Net :</span>
            ...
            <div className="flex justify-between items-center pt-3 border-t-4 border-primary mt-3">
          ...
          <div className="flex flex-col items-center w-40">
            <div className="w-full h-28 border-2 border-dashed border-primary/20 rounded-2xl flex items-center justify-center relative bg-primary/5 overflow-hidden mb-3">
        ...
        <div className="flex justify-center items-center mt-10 pt-6 border-t border-slate-50">
