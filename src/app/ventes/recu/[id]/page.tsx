    ...
    <div className="pdf-a5-portrait bg-white flex flex-col p-[8mm] relative h-[210mm] max-h-[210mm] overflow-hidden">
      ...
      <div className="mb-8 flex-1">
        ...
        <table className="w-full text-[9px]">
          <thead className="bg-slate-800 text-white border-b border-slate-900"><tr><th className="p-2 text-left uppercase tracking-widest">Date</th><th className="p-2 text-right uppercase tracking-widest">Montant</th></tr></thead>
          <tbody>
            {saleData?.payments && saleData.payments.length > 0 ? (saleData.payments.map((p: any, i: number) => {
              ...
              return (<tr key={i} className="border-b border-slate-50"><td className="p-2 font-bold text-slate-600 uppercase">{pDate}</td><td className="p-2 text-right font-black text-slate-900 tabular-nums">{formatCurrency(p.amount)}</td></tr>);
            })) : (<tr className="border-b border-slate-50"><td className="p-2 font-bold text-slate-600 uppercase">{dateDisplay}</td><td className="p-2 text-right font-black text-slate-900 tabular-nums">{formatCurrency(avance)}</td></tr>)}
          </tbody>
        </table>
      </div>

      <div className="space-y-4">
        <div className="w-full space-y-2 border-t border-slate-200 pt-6"><div className="flex justify-between text-[9px] text-slate-500 font-bold uppercase tracking-widest px-2"><span>Commande :</span><span className="tabular-nums font-black">{formatCurrency(totalNet)}</span></div><div className="flex justify-between text-[9px] text-green-600 font-black uppercase tracking-widest px-2"><span>Déjà payé :</span><span className="tabular-nums font-black">{formatCurrency(avance)}</span></div><div className="flex justify-between items-center pt-4 border border-slate-300 bg-slate-50 text-slate-950 p-4 rounded-2xl mt-2"><span className="text-[10px] font-black uppercase tracking-[0.4em]">Reste à Régler</span><span className="text-xl font-black tracking-tighter tabular-nums">{formatCurrency(reste)}</span></div></div>
