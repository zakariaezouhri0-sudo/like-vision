      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent>
              ...
              <DialogFooter><Button onClick={handleCreateClient}>Enregistrer</Button></DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        <Dialog open={!!editingClient} onOpenChange={(open) => !open && setEditingClient(null)}>
          <DialogContent>
            ...
            <DialogFooter><Button onClick={handleUpdateClient}>Sauvegarder</Button></DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Card className="shadow-sm">
          <CardHeader className="py-4 px-6 border-b">
            <div className="relative max-w-sm">
              ...
              <Input placeholder="Nom ou téléphone..." className="pl-10 h-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </CardHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredClients.length > 0 ? (
                      filteredClients.map((c: any) => (
                        <TableRow key={c.id}>
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-2"><div className="h-7 w-7 rounded-full bg-primary/10 flex items-center justify-center"><User className="h-3.5 w-3.5 text-primary" /></div>{c.name}</div>
                          </TableCell>
                          <TableCell className="text-xs font-medium">{c.phone}</TableCell>
                          <TableCell><Badge variant="outline" className="text-[10px]">{c.mutuelle}</Badge></TableCell>
                      ...
                      <TableRow><TableCell colSpan={5} className="text-center py-10">Aucun client.</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              )}
