...
export default function CashSessionsPage() {
  ...
  const groupedSessions = useMemo(() => {
    ...
    // Filtrage et tri initial (décroissant)
                    ...
                    {groupedSessions.length > 0 ? (
                      groupedSessions.map((group) => (
                        <React.Fragment key={group.monthLabel}>
                          {/* En-tête de mois */}
                          <TableRow className="bg-slate-50/80 hover:bg-slate-50/80 border-y border-slate-200/60">
                            <TableCell colSpan={8} className="px-8 py-4">
                              <div className="flex items-center gap-3">
                                <div className="h-2 w-2 rounded-full bg-primary" />
                                <span className="text-sm font-black text-primary uppercase tracking-[0.2em] drop-shadow-sm">
                                  {group.monthLabel}
                                </span>
