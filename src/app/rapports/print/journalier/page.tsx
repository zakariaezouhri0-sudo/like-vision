...
function DailyCashReportContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col items-center pt-4 pb-10 print:pt-0 print:pb-0 print:bg-white">
      ...
      <div className="pdf-a4-portrait shadow-[0_0_60px_rgba(0,0,0,0.05)] bg-white print:shadow-none print:m-0 border border-slate-100 rounded-sm px-[8mm] pb-[8mm] pt-[4mm] print:pt-[4mm] flex flex-col min-h-[297mm]">
        ...
        <div className="flex justify-between items-start border-b border-slate-200 pb-2 mb-3">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[7px] text-slate-500 font-bold leading-tight max-w-[300px]">{shop.address}</p>
              <div className="flex items-center gap-2">
                <span className="text-[7px] font-black text-slate-800">Tél: {shop.phone}</span>
                <span className="h-0.5 w-0.5 rounded-full bg-slate-300" />
                <span className="text-[7px] font-black text-slate-800">ICE: {shop.icePatent}</span>
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="border border-slate-900 px-2 py-0.5 rounded-md inline-block mb-1">
              <h2 className="text-[8px] font-black uppercase tracking-[0.2em] leading-none text-slate-900">Rapport de Caisse</h2>
            </div>
            <div className="space-y-0.5">
              <div className="flex items-center justify-end gap-2 text-[9px] font-black text-slate-900">
        ...
        <div className="grid grid-cols-4 gap-2 mb-3">
          <div className="p-1.5 rounded-lg border border-slate-300 bg-slate-50/30 text-center">
            <p className="text-[6px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Ouverture</p>
            <p className="text-sm font-black text-slate-900 tabular-nums">{formatCurrency(reportData.initial)}</p>
          </div>
          <div className="p-1.5 rounded-lg border border-green-300 bg-green-50/20 text-center">
            <p className="text-[6px] font-black uppercase tracking-widest text-green-600 mb-0.5">Flux (Op)</p>
            <p className={cn("text-sm font-black tabular-nums", reportData.fluxOp >= 0 ? "text-green-700" : "text-red-700")}>
              {reportData.fluxOp > 0 ? "+" : ""}{formatCurrency(reportData.fluxOp)}
            </p>
          </div>
          <div className="p-1.5 rounded-lg border border-orange-300 bg-orange-50/20 text-center">
            <p className="text-[6px] font-black uppercase tracking-widest text-orange-600 mb-0.5">Versements</p>
            <p className="text-sm font-black text-orange-700 tabular-nums">{formatCurrency(reportData.totalVersements)}</p>
          </div>
          <div className="p-1.5 rounded-lg border-2 border-slate-900 bg-white text-center">
            <p className="text-[6px] font-black uppercase tracking-widest text-slate-500 mb-0.5">Solde Clôture</p>
            <p className="text-sm font-black text-slate-950 tabular-nums">{formatCurrency(reportData.final)}</p>
          </div>
        </div>
        ...
        <div className="space-y-3 flex-1">
          <div className="space-y-1">
            <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
              <h3 className="text-[8px] font-black uppercase text-slate-900 flex items-center gap-2 tracking-widest">
                <TrendingUp className="h-2.5 w-2.5 text-green-600" />
                Journal des Encaissements
              </h3>
              <span className="text-[7px] font-black text-green-700 uppercase">
                      ...
                      <tr key={s.id} className="hover:bg-slate-50">
                        <td className="p-1.5 align-middle">
                          <div className="flex flex-col">
                            {s.label && <span className="text-[8px] font-black text-slate-800 uppercase leading-tight">{s.label}</span>}
                            {s.clientName && <span className="text-[6px] font-bold text-slate-400 uppercase tracking-tighter">{s.clientName}</span>}
                          </div>
                        </td>
                        ...
                        <td className="p-1.5 text-center align-middle">
                          {sale && (
                            <span className={cn(
                              "text-[5.5px] px-1 py-0.5 rounded font-black uppercase leading-none inline-flex items-center justify-center",
                        ...
                        <td className="p-1.5 text-right font-black text-slate-950 tabular-nums text-[9px] align-middle">+{formatCurrency(Math.abs(s.montant))}</td>
                      </tr>
                    );
                  }) : (
                    <tr><td colSpan={6} className="p-3 text-center text-slate-300 font-bold italic text-[7px]">Aucune vente enregistrée.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
                ...
                <span className="text-[7px] font-black text-red-700 uppercase">-{formatCurrency(Math.abs(reportData.expenses.reduce((a, b) => a + Math.abs(b.montant || 0), 0)))}</span>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {reportData.expenses.length > 0 ? reportData.expenses.map((e: any) => (
                      <tr key={e.id} className="hover:bg-slate-50">
                        <td className="p-1.5 align-middle">
                          <span className="text-[8px] font-black text-slate-800 uppercase leading-tight">
                            {e.type !== "DEPENSE" ? `${e.type} | ${e.label}` : e.label}
                          </span>
                        </td>
                        <td className="p-1.5 text-center align-middle">
                          <span className="text-[7px] font-bold text-slate-500 uppercase tracking-tighter">{e.clientName || "---"}</span>
                        </td>
                      ...
                      <tr><td colSpan={3} className="p-2 text-center text-slate-300 font-bold italic text-[7px]">Aucune dépense.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {reportData.versements.length > 0 && (
              <div className="space-y-1">
                <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
                  <h3 className="text-[7px] font-black uppercase text-slate-900 flex items-center gap-2 tracking-widest">
                    <Landmark className="h-2.5 w-2.5 text-orange-600" />
                    Versements Détaillés
                  </h3>
                  <span className="text-[7px] font-black text-orange-700 uppercase">-{formatCurrency(Math.abs(reportData.totalVersements))}</span>
                </div>
                <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                  <table className="w-full border-collapse">
                    <thead className="bg-slate-50 text-slate-900 border-b border-slate-200">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {reportData.versements.map((v: any) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          <td className="p-1 text-[7px] font-bold text-slate-900 uppercase leading-tight align-middle">{v.label}</td>
                          <td className="p-1 text-right font-black text-slate-950 tabular-nums text-[8px] align-middle">-{formatCurrency(Math.abs(v.montant))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-2 pt-2 border-t border-slate-200 grid grid-cols-2 gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-slate-400">
              <UserCheck className="h-2.5 w-2.5" />
              <p className="text-[6px] font-black uppercase tracking-widest">Visa du Responsable</p>
            </div>
            <div className="border-b border-slate-200 w-full opacity-50"></div>
            <p className="text-[7px] font-bold text-slate-900">Signature</p>
          </div>
          <div className="text-right flex flex-col items-end">
            <p className="text-[6px] font-black uppercase text-slate-400 tracking-widest mb-1">Authentification</p>
            <div className="w-[40mm] h-[15mm] border border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50/20 relative">
              <span className="text-[6px] text-slate-200 font-black rotate-[-12deg] uppercase tracking-[0.3em] opacity-40 text-center leading-loose select-none px-4">
                CACHE
              </span>
            </div>
          </div>
        ...
        <div className="mt-auto text-center border-t border-slate-50 pt-1">
          <p className="text-[5px] text-slate-400 font-black uppercase tracking-[0.5em] italic opacity-40">
