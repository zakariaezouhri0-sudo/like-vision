      ...
      <div className="pdf-a4-portrait shadow-[0_0_60px_rgba(0,0,0,0.05)] bg-white print:shadow-none print:m-0 border border-slate-100 rounded-sm px-[8mm] pb-[8mm] pt-[4mm] print:pt-[4mm] flex flex-col min-h-[297mm]">
        ...
        <div className="flex justify-between items-start border-b border-slate-200 pb-3 mb-4">
          ...
          <div className="text-right">
            <div className="border border-slate-900 px-2.5 py-1 rounded-md inline-block mb-1.5">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-100">
                  {reportData.sales.length > 0 ? reportData.sales.map((s: any) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-1.5 text-center align-middle">
                          {sale && (
                            <span className={cn(
                              "text-[6px] px-1 py-0.5 rounded font-black uppercase leading-none inline-flex items-center justify-center",
                    ...
                    <tr><td colSpan={6} className="p-3 text-center text-slate-300 font-bold italic text-[8px]">Aucune vente enregistrée.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <div className="flex items-center justify-between border-b border-slate-900 pb-1 px-1">
                ...
                <span className="text-[8px] font-black text-red-700">-{formatCurrency(Math.abs(reportData.expenses.reduce((a, b) => a + Math.abs(b.montant || 0), 0)))}</span>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {reportData.expenses.length > 0 ? reportData.expenses.map((e: any) => (
                      ...
                      <tr><td colSpan={3} className="p-3 text-center text-slate-300 font-bold italic text-[7px]">Aucune dépense.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {reportData.versements.length > 0 && (
              <div className="space-y-1">
                <div className="flex items-center justify-between border-b border-slate-900 pb-1 px-1">
                  ...
                  <span className="text-[8px] font-black text-orange-700">-{formatCurrency(Math.abs(reportData.totalVersements))}</span>
                </div>
                <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                  <table className="w-full">
                    <thead className="bg-slate-50 text-slate-900 border-b border-slate-200">
                      <tr>
                        <th className="p-1.5 text-left text-[7px] font-black uppercase tracking-widest">Désignation</th>
                        <th className="p-1.5 text-right text-[7px] font-black uppercase tracking-widest w-20">Montant</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {reportData.versements.map((v: any) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          <td className="p-1.5 text-[8px] font-bold text-slate-900 uppercase leading-tight align-middle">{v.label}</td>
                          <td className="p-1.5 text-right font-black text-slate-950 tabular-nums text-[8px] align-middle">-{formatCurrency(Math.abs(v.montant))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-4 pt-3 border-t border-slate-200 grid grid-cols-2 gap-10">
          <div className="space-y-6">
            <div className="flex items-center gap-2 text-slate-400">
              <UserCheck className="h-3 w-3" />
              <p className="text-[7px] font-black uppercase tracking-widest">Visa du Responsable</p>
            </div>
            <div className="border-b border-slate-200 w-full opacity-50"></div>
            <p className="text-[8px] font-bold text-slate-900">Cachet & Signature</p>
          </div>
          <div className="text-right flex flex-col items-end">
            <p className="text-[7px] font-black uppercase text-slate-400 tracking-widest mb-1.5">Authentification</p>
            <div className="w-[45mm] h-[20mm] border border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50/20 relative">
              <span className="text-[7px] text-slate-200 font-black rotate-[-12deg] uppercase tracking-[0.3em] opacity-40 text-center leading-loose select-none px-4">
                ESPACE CACHET
              </span>
            </div>
          </div>
