  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col items-center pt-2 pb-6 print:pt-0 print:pb-0 print:bg-white">
      <div className="no-print w-full max-w-[210mm] flex justify-between items-center mb-4 px-4">
        <Button variant="outline" asChild className="bg-white border-slate-200 text-slate-600 h-10 px-4 rounded-[8px] shadow-sm font-black text-[10px] hover:bg-slate-50"><Link href="/caisse"><ArrowLeft className="mr-2 h-4 w-4" /> RETOUR CAISSE</Link></Button>
        <div className="flex items-center gap-3"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-white px-4 py-2 rounded-full border shadow-sm">A4 Portrait</span><Button onClick={() => window.print()} className="bg-[#0D1B2A] shadow-xl hover:bg-[#0D1B2A]/90 h-10 px-8 rounded-[8px] font-black text-xs text-white"><Printer className="mr-2 h-4 w-4" /> IMPRIMER LE RAPPORT</Button></div>
      </div>

      <div className="pdf-a4-portrait shadow-[0_0_60px_rgba(0,0,0,0.05)] bg-white print:shadow-none print:m-0 border border-slate-100 rounded-none px-[6mm] pb-[6mm] pt-[3mm] print:pt-[3mm] flex flex-col min-h-[297mm]">
        <div className="flex justify-between items-start border-b border-slate-200 pb-2 mb-3">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 flex items-center justify-center shrink-0 overflow-hidden relative border border-slate-100 rounded-[4px] bg-white shadow-sm">{shop.logoUrl ? (<img src={shop.logoUrl} alt="Logo" className="h-full w-full object-contain p-1" />) : (<div className="relative text-[#0D1B2A]"><Glasses className="h-8 w-8" /><ThumbsUp className="h-4 w-4 absolute -top-1 -right-1 bg-white p-0.5 rounded-full border border-[#0D1B2A]" /></div>)}</div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-10">
          ...
          <div className="text-right flex flex-col items-end"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">Authentification</p><div className="w-[45mm] h-[18mm] border border-dashed border-slate-300 rounded-[4px] flex items-center justify-center bg-slate-50/20 relative"><span className="text-[9px] text-slate-200 font-black rotate-[-12deg] uppercase tracking-[0.3em] opacity-40 select-none px-4">CACHE</span></div></div>
        </div>
