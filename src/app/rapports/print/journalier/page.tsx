      ...
      <div className="pdf-a4-portrait shadow-[0_0_60px_rgba(0,0,0,0.05)] bg-white print:shadow-none print:m-0 border border-slate-100 rounded-sm px-[6mm] pb-[6mm] pt-[3mm] print:pt-[3mm] flex flex-col min-h-[297mm]">
        ...
        <div className="flex justify-between items-start border-b border-slate-200 pb-2 mb-3">
          <div className="flex items-center gap-4">
            <div className="h-10 w-10 flex items-center justify-center shrink-0 overflow-hidden relative border border-slate-100 rounded-xl bg-white shadow-sm">
              {shop.logoUrl ? (
            ...
            <div className="space-y-0">
              ...
              <p className="text-[7px] text-slate-500 font-bold leading-tight max-w-[250px]">{shop.address}</p>
              <div className="flex items-center gap-2">
                <span className="text-[7px] font-black text-slate-800 uppercase">Tél: {shop.phone} • ICE: {shop.icePatent}</span>
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="border-2 border-slate-900 px-2.5 py-1 rounded-md inline-block mb-1">
              <h2 className="text-[9px] font-black uppercase tracking-[0.2em] leading-none text-slate-900">Rapport de Caisse</h2>
            </div>
            <div className="space-y-0">
              <div className="flex items-center justify-end gap-2 text-[10px] font-black text-slate-900">
                <Calendar className="h-3 w-3 text-primary/40" />
                <span>{format(selectedDate, "dd MMMM yyyy", { locale: fr })}</span>
              </div>
              <div className="flex items-center justify-end gap-2 text-[6px] font-bold text-slate-400 italic">
        ...
        <div className="grid grid-cols-4 gap-2.5 mb-3">
          <div className="p-2 rounded-lg border border-slate-300 bg-slate-50/30 text-center">
            <p className="text-[7px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Ouverture</p>
            <p className="text-base font-black text-slate-900 tabular-nums">{formatCurrency(reportData.initial)}</p>
          </div>
          <div className="p-2 rounded-lg border border-green-300 bg-green-50/20 text-center">
            <p className="text-[7px] font-black uppercase tracking-widest text-green-600 mb-0.5">Flux (Op)</p>
            <p className={cn("text-base font-black tabular-nums", reportData.fluxOp >= 0 ? "text-green-700" : "text-red-700")}>
              {reportData.fluxOp > 0 ? "+" : ""}{formatCurrency(reportData.fluxOp)}
            </p>
          </div>
          <div className="p-2 rounded-lg border border-orange-300 bg-orange-50/20 text-center">
            <p className="text-[7px] font-black uppercase tracking-widest text-orange-600 mb-0.5">Versements</p>
            <p className="text-base font-black text-orange-700 tabular-nums">{formatCurrency(reportData.totalVersements)}</p>
          </div>
          <div className="p-2 rounded-lg border-2 border-slate-900 bg-white text-center">
            <p className="text-[7px] font-black uppercase tracking-widest text-slate-500 mb-0.5">Solde Clôture</p>
            <p className="text-base font-black text-slate-950 tabular-nums">{formatCurrency(reportData.final)}</p>
          </div>
        </div>

        <div className="space-y-3 flex-1">
          <div className="space-y-1">
            <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
            ...
            <div className="overflow-hidden border border-slate-200 rounded-lg">
              <table className="w-full border-collapse">
                <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                  <tr>
                    <th className="p-1.5 text-left text-[9px] font-black uppercase tracking-widest w-[30%]">Document / Client</th>
                    <th className="p-1.5 text-center text-[9px] font-black uppercase tracking-widest w-28">Total Net</th>
                    <th className="p-1.5 text-center text-[9px] font-black uppercase tracking-widest w-28 text-green-600">Versé</th>
                    <th className="p-1.5 text-center text-[9px] font-black uppercase tracking-widest text-destructive w-28">Reste</th>
                    <th className="p-1.5 text-center text-[9px] font-black uppercase tracking-widest w-24">Statut</th>
                    <th className="p-1.5 text-right text-[9px] font-black uppercase tracking-widest w-36">Acompte Jour</th>
                  </tr>
                </thead>
                  ...
                  {reportData.sales.length > 0 ? reportData.sales.map((s: any) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-1.5 text-center font-bold text-slate-600 tabular-nums text-[11px] align-middle">{sale ? formatCurrency(totalNet) : "---"}</td>
                        <td className="p-1.5 text-center font-bold text-green-600 tabular-nums text-[11px] align-middle">{sale ? formatCurrency(sale.avance || 0) : "---"}</td>
                        <td className="p-1.5 text-center font-bold text-destructive tabular-nums text-[11px] align-middle">{sale ? formatCurrency(sale.reste || 0) : "---"}</td>
                        ...
                        <td className="p-1.5 text-right font-black text-slate-950 tabular-nums text-[12px] align-middle">+{formatCurrency(Math.abs(s.montant))}</td>
                      </tr>
                    );
                  }) : (
                    <tr><td colSpan={6} className="p-3 text-center text-slate-300 font-bold italic text-[10px]">Aucune vente enregistrée.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
                ...
                <span className="text-[10px] font-black text-red-700 uppercase">-{formatCurrency(Math.abs(reportData.expenses.reduce((a, b) => a + Math.abs(b.montant || 0), 0)))}</span>
              </div>
              <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                <table className="w-full border-collapse">
                  <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                    <tr>
                      <th className="p-1.5 text-left text-[10px] font-black uppercase tracking-widest w-[35%]">Désignation / Nature</th>
                      <th className="p-1.5 text-left text-[10px] font-black uppercase tracking-widest">Bénéficiaire / Client</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {reportData.expenses.length > 0 ? reportData.expenses.map((e: any) => (
                      <tr key={e.id} className="hover:bg-slate-50">
                        <td className="p-1.5 align-middle text-left">
                        ...
                        <td className="p-1.5 text-left align-middle">
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{e.clientName || "---"}</span>
                        </td>
                        <td className="p-1.5 text-right font-black text-slate-950 tabular-nums text-[11px] align-middle">-{formatCurrency(Math.abs(e.montant))}</td>
                      </tr>
                    )) : (
                      <tr><td colSpan={3} className="p-2 text-center text-slate-300 font-bold italic text-[10px]">Aucune dépense.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {reportData.versements.length > 0 && (
              <div className="space-y-1">
                <div className="flex items-center justify-between border-b border-slate-900 pb-0.5 px-1">
                  ...
                  <span className="text-[10px] font-black text-orange-700 uppercase">-{formatCurrency(Math.abs(reportData.totalVersements))}</span>
                </div>
                <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                  <table className="w-full border-collapse">
                    <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {reportData.versements.map((v: any) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          <td className="p-1.5 text-[11px] font-bold text-slate-900 uppercase leading-tight align-middle">{v.label}</td>
                          <td className="p-1.5 text-right font-black text-slate-950 tabular-nums text-[11px] align-middle">-{formatCurrency(Math.abs(v.montant))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-3 pt-3 border-t border-slate-200 grid grid-cols-2 gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-slate-400">
              <UserCheck className="h-3 w-3" />
              <p className="text-[8px] font-black uppercase tracking-widest">Visa du Responsable</p>
            </div>
            <div className="border-b border-slate-200 w-full opacity-50"></div>
            <p className="text-[10px] font-bold text-slate-900">Signature</p>
          </div>
          <div className="text-right flex flex-col items-end">
            <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest mb-1">Authentification</p>
            <div className="w-[40mm] h-[15mm] border border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50/20 relative">
              <span className="text-[8px] text-slate-200 font-black rotate-[-12deg] uppercase tracking-[0.3em] opacity-40 text-center leading-loose select-none px-4">
                CACHE
              </span>
            </div>
          </div>
        ...
        <div className="mt-auto text-center border-t border-slate-50 pt-1">
          <p className="text-[6px] text-slate-400 font-black uppercase tracking-[0.5em] italic opacity-40">
