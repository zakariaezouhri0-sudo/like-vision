      ...
      <div className="pdf-a4-portrait shadow-[0_0_60px_rgba(0,0,0,0.05)] bg-white print:shadow-none print:m-0 border border-slate-100 rounded-sm px-[8mm] pb-[8mm] pt-[4mm] print:pt-[4mm] flex flex-col min-h-[297mm]">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-0.5">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[8px] font-black text-slate-800">Tél: {shop.phone}</span>
                <span className="h-0.5 w-0.5 rounded-full bg-slate-300" />
                <span className="text-[8px] font-black text-slate-800">ICE: {shop.icePatent}</span>
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="border-2 border-slate-900 px-3 py-1 rounded-md inline-block mb-1.5">
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em] leading-none text-slate-900">Rapport de Caisse</h2>
            </div>
            <div className="space-y-0.5">
              <div className="flex items-center justify-end gap-2 text-[11px] font-black text-slate-900">
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <div className="p-2.5 rounded-lg border border-green-300 bg-green-50/20 text-center">
            <p className="text-[8px] font-black uppercase tracking-widest text-green-600 mb-1">Flux (Op)</p>
          ...
          <div className="p-2.5 rounded-lg border border-orange-300 bg-orange-50/20 text-center">
            <p className="text-[8px] font-black uppercase tracking-widest text-orange-600 mb-1">Versements</p>
          ...
          <div className="p-2.5 rounded-lg border-2 border-slate-900 bg-white text-center">
            <p className="text-[8px] font-black uppercase tracking-widest text-slate-500 mb-1">Solde Clôture</p>
            ...
            <div className="overflow-hidden border border-slate-200 rounded-lg">
              <table className="w-full border-collapse">
                <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                  <tr>
                    <th className="p-2 text-left text-[9px] font-black uppercase tracking-widest">Document / Client</th>
                    <th className="p-2 text-center text-[9px] font-black uppercase tracking-widest w-28">Total Net</th>
                    <th className="p-2 text-center text-[9px] font-black uppercase tracking-widest w-28 text-green-600">Versé</th>
                    <th className="p-2 text-center text-[9px] font-black uppercase tracking-widest text-destructive w-28">Reste</th>
                    <th className="p-2 text-center text-[9px] font-black uppercase tracking-widest w-24">Statut</th>
                    <th className="p-2 text-right text-[9px] font-black uppercase tracking-widest w-32">Acompte Jour</th>
                  </tr>
                </thead>
                  ...
                  {reportData.sales.length > 0 ? reportData.sales.map((s: any) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-2 text-center font-bold text-green-600 tabular-nums text-[10px] align-middle">{sale ? formatCurrency(sale.avance || 0) : "---"}</td>
                        <td className="p-2 text-center font-bold text-destructive tabular-nums text-[10px] align-middle">{sale ? formatCurrency(sale.reste || 0) : "---"}</td>
                        ...
                        <td className="p-2 text-right font-black text-slate-950 tabular-nums text-[11px] align-middle">+{formatCurrency(Math.abs(s.montant))}</td>
                      </tr>
                    );
                  }) : (
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <div className="flex items-center justify-between border-b-2 border-slate-900 pb-1 px-1">
                ...
                <span className="text-[9px] font-black text-red-700 uppercase">-{formatCurrency(Math.abs(reportData.expenses.reduce((a, b) => a + Math.abs(b.montant || 0), 0)))}</span>
              </div>
              <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                <table className="w-full border-collapse">
                  <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                    <tr>
                      <th className="p-2 text-left text-[9px] font-black uppercase tracking-widest w-[45%]">Désignation / Nature</th>
                      <th className="p-2 text-center text-[9px] font-black uppercase tracking-widest">Bénéficiaire / Client</th>
                      <th className="p-2 text-right text-[9px] font-black uppercase tracking-widest w-32">Montant</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reportData.expenses.length > 0 ? reportData.expenses.map((e: any) => (
                      <tr key={e.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-2 text-right font-black text-slate-950 tabular-nums text-[10px] align-middle">-{formatCurrency(Math.abs(e.montant))}</td>
                      </tr>
                    )) : (
                      <tr><td colSpan={3} className="p-3 text-center text-slate-300 font-bold italic text-[9px]">Aucune dépense.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {reportData.versements.length > 0 && (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between border-b-2 border-slate-900 pb-1 px-1">
                  ...
                  <span className="text-[9px] font-black text-orange-700 uppercase">-{formatCurrency(Math.abs(reportData.totalVersements))}</span>
                </div>
                <div className="overflow-hidden border border-slate-200 rounded-lg bg-white">
                  <table className="w-full border-collapse">
                    <thead className="bg-slate-100 text-slate-900 border-b border-slate-200">
                      <tr>
                        <th className="p-2 text-left text-[9px] font-black uppercase tracking-widest">Désignation</th>
                        <th className="p-2 text-right text-[9px] font-black uppercase tracking-widest w-32">Montant</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {reportData.versements.map((v: any) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          <td className="p-2 text-[10px] font-bold text-slate-900 uppercase leading-tight align-middle">{v.label}</td>
                          <td className="p-2 text-right font-black text-slate-950 tabular-nums text-[10px] align-middle">-{formatCurrency(Math.abs(v.montant))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-12">
          ...
          <div className="text-right flex flex-col items-end">
            <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest mb-1.5">Authentification</p>
        ...
        <div className="mt-auto text-center border-t border-slate-50 pt-2">
