  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <aside className="w-80 border-r bg-card hidden md:flex flex-col sticky top-0 h-screen shadow-2xl">
        ...
        <div className="flex-1 py-8 overflow-y-auto px-3">
          <SidebarNav role={role} />
        </div>
        <div className="p-6 border-t mt-auto bg-slate-50/50">
          <div className="flex items-center gap-4 px-5 py-4 bg-white rounded-3xl border border-primary/10 shadow-sm">
            <Avatar className="h-12 w-12 border-4 border-primary/5 shadow-inner">
              <AvatarFallback className="bg-primary/10 text-primary text-base font-black">{userInitials}</AvatarFallback>
            </Avatar>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-24 border-b bg-card flex items-center justify-between px-6 md:px-12 sticky top-0 z-30 shadow-sm backdrop-blur-xl bg-white/80">
          <div className="flex items-center gap-4">
            ...
            <Sheet open={open} onOpenChange={setOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="md:hidden h-12 w-12 hover:bg-primary/10 rounded-2xl">
              ...
              <SheetContent side="left" className="p-0 w-80">
                <SheetHeader className="p-8 border-b text-left bg-slate-50">
                  <SheetTitle className="flex items-center gap-4">
                    <div className="h-14 w-14 bg-primary rounded-2xl flex items-center justify-center text-primary-foreground shadow-xl">
                       <div className="relative">
                    ...
                    <div className="flex flex-col">
                      <span className="font-headline font-black text-3xl text-primary tracking-tighter leading-none">{APP_NAME}</span>
                      <span className="text-[10px] font-black text-primary/40 uppercase tracking-widest mt-1">Optique Pro</span>
                    </div>
                  </SheetTitle>
                </SheetHeader>
                <div className="py-8 overflow-y-auto max-h-[calc(100vh-140px)] px-3" onClick={() => setOpen(false)}>
                  <SidebarNav role={role} />
                </div>
              </SheetContent>
            </Sheet>
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" asChild className="text-slate-500 hover:text-destructive hover:bg-destructive/5 h-12 px-5 rounded-2xl transition-all">
              <Link href="/login">
                <LogOut className="h-5 w-5 md:mr-3" />
                <span className="hidden md:inline text-xs font-black uppercase tracking-widest">Se Déconnecter</span>
              </Link>
            </Button>
          </div>
        </header>
        <main className="flex-1 p-6 md:p-12 overflow-y-auto bg-slate-50/70">
